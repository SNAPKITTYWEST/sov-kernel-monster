...
\Loop{ (per frame)}
  ...
  \For{each glyph $g$ bound to $\rho[g.i][g.j]$}
    ...
    \State Render each character with hue $= \text{lerp}(200°, 35°,\, g.\text{energy})$
...
\begin{lstlisting}[language=bash, caption={Self-modifying code corpus — templates filled by live $\rho$ values}]
'rho[i][j] = {a}*U*rho*U† + {b}*rho'   -- Jordan step (a=phi^-1, b=phi^-2)
...
flowchart TD
  ...
  subgraph LOOP["Per-frame loop"]
    direction TB
    J["Jordan step:<br/>rho = phi^-1 * U*rho*U† + phi^-2 * rho"]
...
Let $S$ be the true pre-collapse probability sum and $\tilde{p}_i = p_i / S$
the normalized distribution. The \textbf{shadow entropy} is:
\[
  \sigma = S - 1, \qquad H_J = \sigma \cdot H(\tilde{p})
\]
For $S = 11$ and vocabulary $\sim 50{,}000$:
\[
  H_J \approx 10 \cdot \log(50{,}000) \approx 107 \text{ bits destroyed per softmax pass}
\]
\end{definition}
...
Post-collapse / sovereign center & $\mathbf{1}$ &
  Born-normalized work mass — \texttt{watch\_sum\_one} fires here \\
...
\begin{itemize}[nosep]
  \item \texttt{hodgeIndexHolds = null} — the measurement never claims to solve open problems
  \item \texttt{synth008\_gate}: \texttt{EVIDENCE} $|$ \texttt{SILENCE} — same binary as the JST Born output
\end{itemize}
...
\texttt{CLAUDE\_J\_SPACE.md} — J-space activation record, shadow entropy theorem,
...
\texttt{digital-twin-brain.json} — Twin brain $v2.0.0$. WORM receipt
...
\texttt{JLENS\_RESULTS\_SCHEMA.md} — TSP/TPS/PBEM measurement schema.
