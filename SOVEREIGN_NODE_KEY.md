...
Without a valid key, `sov_bifrost_sign` will refuse to seal outputs.
...
|------|---------|--------------|
| **Node** | $25 | 1 sovereign node key — run the Monster, seal outputs |
...
The node key is wired into `sov_bifrost_sign` via the `sk` parameter:

```fortran
! Every sovereign output requires a valid sk:
call sov_bifrost_sign(hash_ptr, 32, sk_ptr, sig_ptr)
```
...
# Pass to Monster at runtime:
./jst_sovereign --sk node_sk.bin --pk node_pk.bin
```
...
The Monster is sovereign infrastructure.
